...
def get_sigmas_karras(
    n: int, sigma_min: float, sigma_max: float, rho: float = 7.0, device="cpu"
) -> Tensor:
...
def get_sigmas_exponential(
    n: int, sigma_min: float, sigma_max: float, device="cpu"
) -> Tensor:
...
class DPMSolver(nn.Module):
    ...
    def dpm_solver_fast(
        self, x: Tensor, t_start, t_end, nfe, eta=0.0, s_noise=1.0
    ) -> Tensor:
