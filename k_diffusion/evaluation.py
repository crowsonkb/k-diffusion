...
class InceptionV3FeatureExtractor(nn.Module):
    def __init__(self):
        super().__init__()
        model = models.inception_v3(pretrained=True).eval().requires_grad_(False)
...
def compute_features(accelerator, sample_fn, extractor_fn, n, batch_size):
    ...
    feats_all = []
    for i in trange(0, n_per_proc, batch_size, disable=not accelerator.is_local_main_process):
        cur_batch_size = min(n - i, batch_size)
        samples = sample_fn(cur_batch_size)[:cur_batch_size]
        feats_all.append(accelerator.gather(extractor_fn(samples)).cpu())
...
def fid(x, y, eps=1e-8):
    ...
    x_cov = torch.cov(x.T)
    y_cov = torch.cov(y.T)
    eps_eye = torch.eye(x_cov.shape[0]) * eps
